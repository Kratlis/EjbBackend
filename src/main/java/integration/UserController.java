...
public class UserController {
        ...
        try {
            if (username == null || password == null)
                return Response.status(Response.Status.BAD_REQUEST).entity(
                        new Message("Username or password is null")).build();
            ...
            return Response.status(Response.Status.UNAUTHORIZED).entity(
                    new Message("Wrong user or password")).build();
        }
    }
    ...
    public Response logout(@HeaderParam("Authorization") String authorization) {
        ...
        try {
            ...
            return Response.ok(new Message("logout successful")).build();
        } catch (Exception e) {
    ...
    public Response register(UserData userData) {
        ...
        if (userService.findUserByUsername(username) != null)
            return Response.status(Response.Status.CONFLICT).entity(
                    new Message("User already exists")).build();
        ...
        return Response.ok(new Message("User successfully registered")).build();
    }
